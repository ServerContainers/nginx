...
export NGINX_VERSION=$(docker run --rm -t get-version dpkg --list nginx-extras | grep '^ii' | sed 's/^[^0-9]*//g' | cut -d ' ' -f1 | sed 's/[+=]/_/g' | tr -d '\r')
[ -z "$DEBIAN_VERSION" ] && exit 1

export IMGTAG=$(echo "$1""d$DEBIAN_VERSION-n$NGINX_VERSION")
